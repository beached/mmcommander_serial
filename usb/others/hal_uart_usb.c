...
*
* @param   uint8_t* buf - buffer with data to write
*          uint16_t length - number of bytes to write
*
* @return  number of bytes written
*/
uint16_t halUartBufferedWrite( const uint8_t* buf, uint16_t length ) {
	uint16_t n;
...
*
* @param   uint8_t* buf - buffer with data to read in to
*          uint16_t length - number of bytes to read
*
* @return  none
*/
uint16_t halUartRead( uint8_t* buf, uint16_t length ) {
	return bufGet( &rbRxBuf, (uint8_t *)buf, length );
}
